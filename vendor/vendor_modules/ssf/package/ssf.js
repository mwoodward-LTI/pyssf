...
function make_ssf(SSF) {
    ...
    var general_fmt_num = (function make_general_fmt_num() {
        ...
        function normalize_exp(o) {
            ...
            return o.replace(mantissa_zeroes_and_decimal, "$1E").replace(
                exp_with_single_digit,
                "$10$2",
            );
        }
    ...
    function write_date(type, fmt, val, ss0) {
        ...
        switch (type) {
            ...
            case 115: /* 's' seconds */
                ...
                ss = roundHalfAwayFromZero(tt * (val.S + val.u));
                if (ss >= 60 * tt) ss = 0;
    ...
    var write_num = (function make_write_num() {
        ...
        function write_num_exp(fmt, val) {
            ...
            if (fmt.match(/^#+0.0E\+0$/)) {
                ...
                if (o.indexOf("e") === -1) {
                    ...
                    if (o.indexOf(".") === -1) {
                        o = o.charAt(0) + "." + o.substr(1) + "E+" +
                            (fakee - o.length + ee);
                    } else o += "E+" + (fakee - ee);
                    while (o.substr(0, 2) === "0.") {
                        o = o.charAt(0) + o.substr(2, period) + "." +
                            o.substr(2 + period);
                        o = o.replace(/^0+([1-9])/, "$1").replace(
                            /^0+\./,
                            "0.",
                        );
                    }
        ...
        function write_num_f1(r, aval, sign) {
            var den = parseInt(r[4], 10),
                rr = roundHalfAwayFromZero(aval * den),
        ...
        function rnd(val, d) {
            ...
            return "" + (roundHalfAwayFromZero(val * dd) / dd);
        }
        function dec(val, d) {
            var _frac = val - Math.floor(val), dd = Math.pow(10, d);
            if (d < ("" + roundHalfAwayFromZero(_frac * dd)).length) return 0;
            return roundHalfAwayFromZero(_frac * dd);
        }
        ...
        function write_num_flt(type, fmt, val) {
            ...
            if ((r = fmt.match(dec1))) {
                o = rnd(val, r[1].length).replace(
                    ...
                    .replace(/\.$/, "." + hashq(r[1])).replace(
                        ...
                        function ($$, $1) {
                            return "." + $1 +
                                fill("0", hashq(r[1]).length - $1.length);
                        },
                    );
            ...
            if ((r = fmt.match(/^(0*)\.(#*)$/))) {
                return sign +
                    rnd(aval, r[2].length).replace(/\.(\d*[1-9])0*$/, ".$1")
                        .replace(
                            /^(-?\d*)$/,
                            "$1.",
            ...
            if ((r = fmt.match(/^00,000\.([#0]*0)$/))) {
                ...
                return val < 0
                    ? "-" + write_num_flt(type, fmt, -val)
                    : commaify(flr(val)).replace(/^\d,\d{3}$/, "0$&").replace(
                        /^\d*$/,
                        function ($$) {
                            return "00," +
        ...
        function write_num_int(type, fmt, val) {
            ...
            if ((r = fmt.match(/^(0*)\.(#*)$/))) {
                return sign +
                    ("" + aval).replace(/\.(\d*[1-9])0*$/, ".$1").replace(
                        /^(-?\d*)$/,
                        "$1.",
                    ).replace(/^0\./, r[1].length ? "0." : ".");
            }
            ...
            if ((r = fmt.match(/^00,000\.([#0]*0)$/))) {
                return val < 0
                    ? "-" + write_num_int(type, fmt, -val)
                    : commaify("" + val).replace(/^\d,\d{3}$/, "0$&").replace(
                        /^\d*$/,
                        function ($$) {
                            return "00," +
                                ($$.length < 3 ? pad0(0, 3 - $$.length) : "") +
    ...
    function fmt_is_date(fmt) {
        ...
        while (i < fmt.length) {
            switch ((c = fmt.charAt(i))) {
                ...
                case "b":
                    if (
                        fmt.charAt(i + 1) === "1" || fmt.charAt(i + 1) === "2"
                    ) {
                        return true;
                    }
                /* falls through */
    ...
    function eval_fmt(fmt, v, opts, flen) {
        ...
        switch (bt) {
            ...
            case 4:
                switch (ss0) {
                    case 1:
                        dt.u = roundHalfAwayFromZero(dt.u * 10) / 10;
                        break;
                    case 2:
                        dt.u = roundHalfAwayFromZero(dt.u * 100) / 100;
                        break;
                    case 3:
                        dt.u = roundHalfAwayFromZero(dt.u * 1000) / 1000;
                        break;
                }
                if (dt.u >= 1) {
                    dt.u = 0;
    ...
    function format(fmt, v, o) {
        ...
        switch (typeof fmt) {
            ...
            case "number":
                ...
                if (sfmt == null) {
                    sfmt = (o.table && o.table[default_map[fmt]]) ||
